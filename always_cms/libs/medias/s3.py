...
class S3:
	...
	def __init__(self):
		self.s3_instance = boto3.resource(
	    's3',
	...
	def get(self, filename):
		return self.s3_instance.Object(configurations.get('s3_bucket_name').value, str(filename)).get()


	def upload(self, filename, content):
		try:
			obj = self.s3_instance.Object(configurations.get('s3_bucket_name').value, str(filename))
	...
	def delete(self, filename):
		try:
			obj = self.s3_instance.Object(configurations.get('s3_bucket_name').value, str(filename)).delete()
			return True
		except ClientError:
			return False

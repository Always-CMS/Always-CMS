...
class Plugin(object):
    ...
    def install_extension(package):
        if hasattr(pip, 'main'):
            result = pip.main(['install', '--quiet', package])
        else:
            result = pip._internal.main(['install', '--quiet', package])
...
def add_filter(event, callback, position='after'):
...
def do_filter(event, *args, **kwargs):
    ...
    result = None
    ctx = stack.top
    for x in ctx.app.extensions.get('plugin_manager')._event_manager.iter(event):
        if result is None:
            result = x(*args, **kwargs)
        else:
            result = x(result, **kwargs)
    if result is None:
        return args[0]
    return result
...
def remove_filter(event, callback):
    ctx = stack.top
    return ctx.app.extensions.get('plugin_manager')._event_manager.remove(event, callback)


def add_event(event, callback, position='after'):
    """Connect a callback to an event.  Per default the callback is
    appended to the end of the handlers but handlers can ask for a higher
    privilege by setting `position` to ``'before'``.
...
def do_event(event, *args, **kwargs):
    ...
    This is equivalent to the following call to :func:`iter_listeners`::
        ...
        for listener in iter_listeners(event):
            result.append(listener(*args, **kwargs))
    """
    do_filter(event, *args, **kwargs)


def remove_event(event, callback):
    return remove_filter(event, callback)


def add_header(callback, position='after'):
    """Connect a callback to an event.  Per default the callback is
    appended to the end of the handlers but handlers can ask for a higher
    privilege by setting `position` to ``'before'``.
...
def do_header(*args, **kwargs):
    ...
    ctx = stack.top
    result = ""
    for x in ctx.app.extensions.get('plugin_manager')._event_manager.iter('header'):
        result += x(*args, **kwargs)
    return result


def remove_header(callback):
    return remove_filter('header', callback)


def add_footer(callback, position='after'):
    """Connect a callback to an event.  Per default the callback is
    appended to the end of the handlers but handlers can ask for a higher
    privilege by setting `position` to ``'before'``.
...
def do_footer(*args, **kwargs):
    ...
    ctx = stack.top
    result = ""
    for x in ctx.app.extensions.get('plugin_manager')._event_manager.iter('footer'):
        result += x(*args, **kwargs)
    return result


def remove_footer(callback):
    return remove_filter('footer', callback)


def add_field_new_post(callback, position='after'):
    """Connect a callback to an event.  Per default the callback is
    appended to the end of the handlers but handlers can ask for a higher
    privilege by setting `position` to ``'before'``.
...
def remove_field_new_post(callback):
    return remove_filter('field_new_post', callback)
...
class EventManager(object):
    ...
    def template_emit(self, event, *args, **kwargs):
        ...
        for f in self.iter(event):
            ...
            if rv is not None:
                results.append(rv)
        return Markup(PluginEventResult(results))


class PluginEventResult(list):
...
class AppPlugin(Plugin):
    ...
    def register_blueprint(self, blueprint, **kwargs):
        ...
        blueprint.name = 'plugin_{}'.format(self.identifier)
